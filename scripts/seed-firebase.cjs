...
// Usage: node scripts/seed-firebase.cjs
...
async function seed() {
  const allowSeed = (process.env.SEED_SAMPLE_ROOMS === 'true') || (process.env.VITE_SEED_SAMPLE_ROOMS === 'true');
  if (!allowSeed) {
    console.log('Skipping seeding. Set SEED_SAMPLE_ROOMS=true to enable sample data.');
    process.exit(0);
  }
